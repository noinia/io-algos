...
-- embed s = \case
--     Leaf' k v   -> singleton $ Embedded.Leaf (k,v)
--     Node' h top -> let top'  = ibimap (f (s + size top) (Complete.size h)) top
--                        top'' = second (fmap fst) top'
--                    in embedTop s top'' <> foldMap1 (foldMap1 snd) top'
--   where
--     -- computes the indices corresponding to the trees l and r
--     f s' sb i (Two l r) = let li = s' + i*2*sb
--                               ri = li + sb
--                           in Two (li,embed' li l) (ri,embed' ri r)


-- embed'   :: Index -> VEBTree TopVeb k v -> NonEmpty (Embedded.Node k (k,v))
-- embed' s = embed s . TopVeb
...
--   \case
--   Leaf' k (Two l r) -> singleton $ Embedded.Node l k r
--   Node' _ chs       -> fmap cleanLeaf $ embed' chs
...
cleanLeaf = \case
  ...
  -- let top' = imap (\i (k, Two l r)) top



  -- foldVEB (singleton . Embedded.Leaf) node t
  -- where
  --   node h top = embed s $ top <>
